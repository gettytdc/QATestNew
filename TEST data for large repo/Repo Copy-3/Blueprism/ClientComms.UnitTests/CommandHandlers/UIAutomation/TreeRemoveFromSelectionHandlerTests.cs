...
{
    internal class TreeRemoveFromSelectionHandlerTests : UIAutomationHandlerTestBase<TreeRemoveFromSelectionHandler>
    {
        ...
        [Test]
        public void ExecuteWithValidElementRemovesFromSelection()
        {
            Execute(GetQuery("Item 1"));
            GetMock<ISelectionItemPattern>().Verify(m => m.RemoveFromSelection(), Times.AtLeastOnce());
        }
        ...
        {
            return clsQuery.Parse($"UIATreeRemoveFromSelection IDName=\"{name}\" Position={matchIndex}");
        }
