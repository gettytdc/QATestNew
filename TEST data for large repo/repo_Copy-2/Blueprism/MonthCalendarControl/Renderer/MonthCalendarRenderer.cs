namespace CustomControls
{
   ...
   public class MonthCalendarRenderer : MonthCalendarAbstractRenderer
   {
         ...
         MonthCalendarHeaderState state)
      {
         ...
         string month = GetMonthForCalendar(firstVisible, date);
         int year = GetYearForCalendar(firstVisible, date);
         DateTime parsedYear = GetParsedYearForCalendar(firstVisible, date);
         ...
         string yearString = this.monthCal.UseNativeDigits
            ...
            : parsedYear.ToString(CultureInfo.CurrentUICulture.DateTimeFormat.YearMonthPattern);
         ...
         int yearX = this.monthCal.UseNativeDigits
            ? x + calMonth.TitleMonthBounds.Width + 7
            : Convert.ToInt32(x + (yearSize.Width * 0.4));
            ...
            brushSelected = new SolidBrush(colorTable.HeaderSelectedText))
         {
            ...
            if (this.monthCal.UseNativeDigits)
            {
         ...
         {
            // get arrow color
            Color arrowColor = GetArrowColour(colorTable, this.monthCal.LeftButtonState);
         ...
         {
            // get arrow color
            Color arrowColor = GetArrowColour(colorTable, this.monthCal.RightButtonState);
        ...
        private string GetMonthForCalendar(MonthCalendarDate firstVisible, MonthCalendarDate date)
        {
            if (firstVisible.Era != date.Era)
            {
                return this.monthCal.FormatProvider.GetMonthName(firstVisible.Year, firstVisible.Month);
            }

            return this.monthCal.FormatProvider.GetMonthName(date.Year, date.Month);
        }

        private int GetYearForCalendar(MonthCalendarDate firstVisible, MonthCalendarDate date)
        {
            if (firstVisible.Era != date.Era)
            {
                return firstVisible.Year;
            }

            return date.Year;
        }

        private DateTime GetParsedYearForCalendar(MonthCalendarDate firstVisible, MonthCalendarDate date)
        {
            if (firstVisible.Era != date.Era)
            {
                return DateTime.ParseExact(firstVisible.ToString(CultureInfo.InvariantCulture), "MM/dd/yyyy", CultureInfo.CurrentUICulture);
            }

            return DateTime.ParseExact(date.ToString(CultureInfo.InvariantCulture), "MM/dd/yyyy", CultureInfo.CurrentUICulture);
        }

        private Color GetArrowColour(MonthCalendarColorTable colorTable, ButtonState buttonState)
        {
