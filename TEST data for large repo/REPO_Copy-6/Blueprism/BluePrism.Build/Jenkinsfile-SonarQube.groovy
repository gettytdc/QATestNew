        ...
        stage('BuildAndSQScan') {
            ...
            try {
                withSonarQubeEnv("${sonarQubeInstanceName}") {
                    bat label: "SonarStart", script: "${sqScannerHome}\\SonarScanner.MSBuild.exe begin /k:${params.SonarQubeProjectKey} /v:#${BUILD_ID} /d:sonar.cs.dotcover.reportsPaths=\"${WORKSPACE}\\Output\\dotCover.Output.html\""
                    ...
                    try {
                        ...
                        bat label: "MSBuild", script: "\"${tool 'VS2017'}\" BluePrism.Build\\build.targets /p:Configuration=Debug /p:BluePrismPlatforms=x64 /p:UnitTestCoverageEnabled=true /maxcpucount /nodeReuse:false"
                    }
                    catch (Exception e) {
